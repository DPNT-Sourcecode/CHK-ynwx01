...
def mix_and_match(aggregated_skus):
    ...
    for mix_and_match_group in MIX_AND_MATCH:
        ...
        mix_and_matches = sum(aggregated_skus.get(sku, 0) for sku in skus) // quantity
        quantity_items_to_remove = mix_and_matches * quantity
        ...
        for n in range(quantity_items_to_remove, 0, -1):
            ...
            while not aggregated_skus.get(ordered_skus[i], 0):
                i += 1
            if aggregated_skus.get(ordered_skus[i], 0):
